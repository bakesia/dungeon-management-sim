...
describe('construction menu groups', () => {
  it('shows available facilities first and sorts locked facilities by unlock tier', () => {
    ...
    expect(tierOne.available.map((facility) => facility.id)).toEqual(['quarters', 'mine', 'fungus_farm', 'warehouse', 'guard_post'])
    expect(tierOne.locked.map((facility) => facility.requiredTier)).toEqual([2, 2, 2, 3, 3])
    expect(tierOne.locked[0]?.name).toBe('마력실')
  })

  it('reveals facilities immediately when the current tier changes', () => {
    expect(getConstructionMenuGroups(1).available.some((facility) => facility.id === 'guard_post')).toBe(true)
