...
import { getPopulationSpace } from '../../engine/population/populationMetrics'
...
export function NpcHub({ state, onPurchase, onHire, onRecruit, onService, onRepair, initialFeature = null }: NpcHubProps) {
  ...
  const [feature, setFeature] = useState<FeatureId | null>(
    initialFeature && isFeatureUnlocked(state, initialFeature) ? initialFeature : firstUnlocked,
  )
  ...
  const getCapacityStatus = (itemId: string) => {
    ...
    const previews = item?.effects.flatMap((effect) => effect.type === 'addResource' && effect.amount > 0
  ...
  return <div className="npc-hub">
    ...
    {feature && isFeatureUnlocked(state, feature) && <section className="npc-feature">
      ...
      {feature === 'shop' && <div className="service-list">{state.shop.offerings.map((offering) => { const item = shopItemDefinitionById[offering.itemId]; if (!item) return null; const capacity = getCapacityStatus(item.id); return <button key={item.id} type="button" disabled={offering.stock <= 0 || !canAfford(state, item.cost) || capacity.blocked} onClick={() => onPurchase(item.id)}><span><strong>{item.name}</strong><small>{item.description} · 재고 {offering.stock}</small>{capacity.text && <small className={capacity.blocked ? 'capacity-warning' : ''}>{capacity.blocked ? '저장 공간 부족 · ' : ''}{capacity.text}</small>}</span><em>{formatResourceCost(item.cost)}</em></button> })}</div>}
      {feature === 'tavern' && <><div className="npc-feature-tabs"><button type="button" className={tavernView === 'mercenary' ? 'is-active' : ''} onClick={() => setTavernView('mercenary')}>용병</button><button type="button" className={tavernView === 'recruitment' ? 'is-active' : ''} onClick={() => setTavernView('recruitment')}>주민 모집</button></div>{tavernView === 'mercenary' ? <div className="service-list">{state.tavern.offers.map((id) => { const contract = mercenaryDefinitionById[id]; if (!contract) return null; const active = state.activeMercenaries.some((entry) => entry.contractId === id && state.day < entry.expiresOnDay); return <button key={id} type="button" disabled={active || !canAfford(state, contract.cost)} onClick={() => onHire(id)}><span><strong>{contract.name}</strong><small>방어 +{contract.combatPower} · {contract.durationDays} DAY{active ? ' · 고용 중' : ''}</small></span><em>{formatResourceCost(contract.cost)}</em></button> })}</div> : <div className="service-list recruitment-list">{state.tavern.recruitmentOffers.map((offer) => { const definition = recruitmentOfferDefinitionById[offer.offerId]; if (!definition) return null; const race = raceDefinitionById[definition.raceId]; const capacityBlocked = getPopulationSpace(state) < definition.count; return <button key={offer.offerId} type="button" disabled={offer.remaining <= 0 || capacityBlocked || !canAfford(state, definition.cost)} onClick={() => onRecruit(offer.offerId)}><RaceIcon iconId={race?.iconId ?? ''} name={race?.name ?? definition.raceId} size={34} /><span><strong>{definition.name}</strong><small>{definition.description}</small><small>{capacityBlocked ? `숙소 부족 · 빈 공간 ${definition.count} 필요` : `남은 모집 ${offer.remaining}회 · 영구 주민 +${definition.count}`}</small></span><em>{formatResourceCost(definition.cost)}</em></button> })}</div>}</>}
